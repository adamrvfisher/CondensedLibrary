...
#This is a two asset portfolio strategy back tester.
#It list all possible asset pairs and performs brute force optimization 
#Using a price relative moving average signal that alters portfolio allocation
...
for m in MajorList:
    ...
    for i in iterations:
        ...
        Asset1['Pass'] = (Asset1['LogRet'] * Asset1['Position'].shift(1))
        ...
        Asset2['Position'] = np.where(Asset1['PriceRelative'].shift(1) > Asset1['PRMA'].shift(1),
                                        d,b)
        #Apply position size to returns to pass to portfolio
        Asset2['Pass'] = (Asset2['LogRet'] * Asset2['Position'].shift(1))
        ...
        Portfolio['Multiplier'] = Portfolio['Multiplier'].fillna(0)
...
Asset3['Pass'] = (Asset3['LogRet'] * Asset3['Position'].shift(1))
...
#Apply position size to pass to portfolio
Asset4['Pass'] = (Asset4['LogRet'] * Asset4['Position'].shift(1))
...
Portfolio2['Multiplier'] = Portfolio2['Multiplier'].fillna(0)

#Portfolio drawdown
MultiplierMax2 = Portfolio2['Multiplier'].cummax()
Drawdown2 = (Portfolio2['Multiplier']/MultiplierMax2) - 1
Drawdown2 = Drawdown2.fillna(0)
PortfolioDrawdown = abs(min(Drawdown2.cummin()))

#Top parameters
print(kfloat)
print('--------')
print(Dataset2[kfloat])
print('Max Drawdown is ', str(PortfolioDrawdown) ,'See Dataset2')
