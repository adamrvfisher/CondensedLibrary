...
#This is a HTML scraper and techincal analysis tool -- Yahoo server side changes to API will yield 4xx responses
...
def TickerToDailyCSV(Ticker):
    ...
    Asset.to_csv(DL + "\\DataSources\\YahooSource\\TimeSeriesData\\DAY-" + Ticker + ".csv")
...
def TickerToWeeklyCSV(Ticker):
    ...
    #Assign URL for post request
    url = ('http://finance.yahoo.com/quote/' + Ticker + '/history?period1=-2552017587' +
           '&period2=2552397400&interval=1d&filter=history&frequency=1wk')
    #Make post request
    page = requests.post(url)
    print(page)
    #Format response to text
    ...
    #Save to preprocess storage
    Asset.to_csv((DL + "\\DataSources\\YahooSource\\TimeSeriesData\\WEK-" + Ticker + ".csv"))
...
def TickerToMonthlyCSV(Ticker):
    ...
    #Assign URL for post request
    url = ('http://finance.yahoo.com/quote/' + Ticker + '/history?period1=-2552017587' +
           '&period2=2552397400&interval=1d&filter=history&frequency=1mo')
    #Make post request
    page = requests.post(url)
    print(page)
    #Format response to text
    ...
    #Save to preprocess storage
    Asset.to_csv((DL + "\\DataSources\\YahooSource\\TimeSeriesData\\MON-" + Ticker + ".csv"))
...
def TickerToDividendCSV(Ticker):
    ...
    url = ('http://finance.yahoo.com/quote/' + Ticker + 
               '/history?period1=-2552017587&period2=2552397400&interval=div%7Csplit&filter=div&frequency=1d')
    #Make post request
    page = requests.post(url)
    print(page)
    #Format response to text
    ...
    #Locate text in response
    markerI = textI.find('[{"amount"')
    #Assign trimmed response
    textI = textI[markerI:]
    
    #Locate text in trimmed response
    markerII = textI.find('},"ThemeStore":')
    #Assign trimmed response
    textI = textI[:markerII]
    #text to json
    textI = json.loads(textI)
    ...
    #Rename columns
    Asset = Asset.rename(columns={"amount": "Amount", "data": "Data", "date": "Date", 
                       "type": "Type"})    
    ...
    Asset = Asset.set_index('Date')
    #Save to preprocess storage
    Asset.to_csv((DL + "\\DataSources\\YahooSource\\DividendData\\DIV-" + Ticker + ".csv"))
...
DL = "F:\\Users\\UserName\\DirectoryLocation"
...
#    try:
        ...
        DAY.loc[(TimeStamps,'Dividends')] = DIV['Data']
